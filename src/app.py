...
def callback_on_completion(status: du.UploadStatus):
    ...
    ProsjektPlan = pd.read_sql_query(sql3, conn)
    Planperioder = pd.read_sql_query(sql4, conn)
    Vegnett = pd.read_sql_query(sql5, conn)
    Utbyggingsplan = pd.read_sql_query(sql6, conn)
